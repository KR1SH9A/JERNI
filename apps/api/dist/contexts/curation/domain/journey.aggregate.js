...
class Journey {
    ...
    publish() {
        if (this.status !== 'DRAFT') {
            throw new domain_error_1.DomainError(`Cannot publish a journey that is already ${this.status}.`, 'JOURNEY_NOT_DRAFT');
        }
        if (this._taskDefinitions.length === 0) {
            throw new domain_error_1.DomainError('Cannot publish a journey with no tasks. Add at least one task first.', 'JOURNEY_HAS_NO_TASKS');
        }
    ...
    archive() {
        if (this.status !== 'PUBLISHED') {
            throw new domain_error_1.DomainError(`Only a PUBLISHED journey can be archived. Current status: ${this.status}.`, 'JOURNEY_NOT_PUBLISHED');
        }
    ...
    addTask(task) {
        if (this.status === 'ARCHIVED') {
            throw new domain_error_1.DomainError('Cannot add tasks to an ARCHIVED journey.', 'JOURNEY_ARCHIVED');
        }
        // Enforce orderIndex uniqueness within the aggregate
        const duplicate = this._taskDefinitions.find((t) => t.orderIndex === task.orderIndex);
        if (duplicate) {
            throw new domain_error_1.DomainError(`A task with orderIndex ${task.orderIndex} already exists on this journey.`, 'TASK_ORDER_INDEX_DUPLICATE');
        }
