import { Journey } from '../journey.aggregate';
...
const makeJourney = (overrides: Partial<ConstructorParameters<typeof Journey>[0]> = {}) =>
  new Journey({
    id: JourneyId.of('00000000-0000-0000-0000-000000000001'),
    curatorId: UserId.of('user-a'),
    title: 'Test Journey',
    ...overrides,
  });

const makeTask = (orderIndex = 0, kind: 'MILESTONE' | 'RECURRING' = 'MILESTONE') =>
  new TaskDefinition({
    id: `task-${orderIndex}`,
...
describe('Journey aggregate invariants', () => {
  ...
  describe('publish()', () => {
    ...
    it('throws JOURNEY_HAS_NO_TASKS when publishing with no tasks', () => {
      const j = makeJourney();
      expect(() => j.publish()).toThrowError(
        expect.objectContaining({ code: 'JOURNEY_HAS_NO_TASKS' }),
      );
    });
    ...
    it('throws JOURNEY_NOT_DRAFT when publishing an already-published journey', () => {
      ...
      expect(() => j.publish()).toThrowError(
        expect.objectContaining({ code: 'JOURNEY_NOT_DRAFT' }),
      );
    });
  ...
  describe('archive()', () => {
    ...
    it('throws JOURNEY_NOT_PUBLISHED when archiving a DRAFT', () => {
      const j = makeJourney();
      expect(() => j.archive()).toThrowError(
        expect.objectContaining({ code: 'JOURNEY_NOT_PUBLISHED' }),
      );
    });
  });
  ...
  describe('addTask()', () => {
    ...
    it('throws JOURNEY_ARCHIVED when adding a task to an archived journey', () => {
      const j = makeJourney({ status: 'ARCHIVED' });
      expect(() => j.addTask(makeTask(0))).toThrowError(
        expect.objectContaining({ code: 'JOURNEY_ARCHIVED' }),
      );
    });
    ...
    it('throws TASK_ORDER_INDEX_DUPLICATE when two tasks share an orderIndex', () => {
      ...
      j.addTask(makeTask(0));
      expect(() => j.addTask(makeTask(0))).toThrowError(
        expect.objectContaining({ code: 'TASK_ORDER_INDEX_DUPLICATE' }),
      );
    });
  });
  ...
  describe('isCurator()', () => {
    it('returns true for the curator', () => {
      const j = makeJourney({ curatorId: UserId.of('user-a') });
      expect(j.isCurator(UserId.of('user-a'))).toBe(true);
    });

    it('returns false for a different user', () => {
      const j = makeJourney({ curatorId: UserId.of('user-a') });
      expect(j.isCurator(UserId.of('user-b'))).toBe(false);
    });
  });
...
describe('TaskDefinition invariants', () => {
  ...
  it('throws TASK_UNEXPECTED_RECURRENCE_RULE for MILESTONE task with a rule', () => {
    expect(
      ...
      expect.objectContaining({ code: 'TASK_UNEXPECTED_RECURRENCE_RULE' }),
    );
  });
});
