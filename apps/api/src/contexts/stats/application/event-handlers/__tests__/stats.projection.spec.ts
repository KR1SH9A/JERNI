...
import { TaskUncompletedHandler } from '../task-uncompleted.handler';
import { StatsRepository } from '../../ports/stats.repository';

// ─── Mock ─────────────────────────────────────────────────────────────────────

const mockStatsRepo: jest.Mocked<StatsRepository> = {
...
describe('Stats Projection Handlers', () => {
  ...
  describe('TaskCompletedHandler', () => {
    ...
    beforeEach(() => {
      handler = new TaskCompletedHandler(mockStatsRepo);
    });

    it('upserts daily_stat with delta=+1 for RECURRING task', async () => {
      const event = new TaskCompletedEvent(
        'journey-1', 'user-1', 'task-1', 'RECURRING', '2025-01-15',
      );

      await handler.handle(event);

      expect(mockStatsRepo.upsertDailyStat).toHaveBeenCalledWith(
        'journey-1', 'user-1', 'task-1', '2025-01-15', 1,
      );
    });
    ...
    it('uses today for daily_stat when MILESTONE task (forDate=null)', async () => {
      ...
      const event = new TaskCompletedEvent(
        'journey-1', 'user-1', 'task-2', 'MILESTONE', null,
      );

      await handler.handle(event);

      expect(mockStatsRepo.upsertDailyStat).toHaveBeenCalledWith(
        'journey-1', 'user-1', 'task-2', today, 1,
      );
    });
    ...
    it('upserts all_time_stat with MILESTONE kind and delta=+1', async () => {
      ...
      expect(mockStatsRepo.upsertAllTimeStat).toHaveBeenCalledWith(
        'journey-1', 'user-1', 'MILESTONE', null, 1,
      );
    });
    ...
    it('upserts all_time_stat with RECURRING kind and delta=+1', async () => {
      ...
      expect(mockStatsRepo.upsertAllTimeStat).toHaveBeenCalledWith(
        'journey-1', 'user-1', 'RECURRING', '2025-01-15', 1,
      );
    });
  ...
  describe('TaskUncompletedHandler', () => {
    let handler: TaskUncompletedHandler;

    beforeEach(() => {
      handler = new TaskUncompletedHandler(mockStatsRepo);
    });

    it('upserts daily_stat with delta=-1 for RECURRING uncomplete', async () => {
      const event = new TaskUncompletedEvent(
        'journey-1', 'user-1', 'task-1', '2025-01-15',
      );
